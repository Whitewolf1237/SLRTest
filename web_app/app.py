...
# Initialize MediaPipe Hands with a higher detection confidence
mp_hands = mp.solutions.hands
mp_drawing = mp.solutions.drawing_utils
mp_drawing_styles = mp.solutions.drawing_styles
hands = mp_hands.Hands(static_image_mode=True, min_detection_confidence=0.6)  # Set to 0.6 or higher
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            if len(data_aux) == 84:  # Expecting 84 features (42 per hand)
                ...
                print(f"Invalid number of features: {len(data_aux)}. Skipping this frame.")
...
def upload_image():
    ...
    # Read the uploaded file into bytes and decode it into an image
    file_bytes = np.frombuffer(file.read(), np.uint8)
    img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)

    if img is None:
        return jsonify({'error': 'Failed to load image. Please check the file format.'}), 400
    
    print("Image loaded successfully")

    # Resize the image to a consistent size for detection
    img = cv2.resize(img, (640, 480))  # Resize to a consistent size for detection
    ...
    if results.multi_hand_landmarks:
        print("Hands detected in uploaded image!")
        for hand_landmarks in results.multi_hand_landmarks:
            x_ = []
            y_ = []
        ...
        if len(data_aux) == 84:
            print(f"Number of features: {len(data_aux)}")
            prediction = model.predict([np.asarray(data_aux)])
            predicted_character = label_encoder.inverse_transform(prediction)[0]
            return jsonify({'prediction': predicted_character})
        else:
            print(f"Invalid number of features detected: {len(data_aux)}")
            return jsonify({'error': 'Invalid feature count for prediction'}), 400
    else:
        print("No hand landmarks detected in the uploaded image.")
